...
description: Use when reviewing a UX spec, mockups, screens, a design doc, or a live UI for usability and UX quality, case & state coverage, and completeness. Evaluates against Nielsen's heuristics, Laws of UX, interaction patterns, information architecture, and content quality, then writes a verdict-first critique report. Invoke for "review this design/spec/mockup", "is this UX any good", "heuristic evaluation", "what states/cases are missing", "critique this flow". Read-only — produces a report, changes nothing.
argument-hint: "<path or URL to the design / spec / mockups to review>"
---
...
Built-in: Read, Write, Edit, WebFetch (read local mockups/screenshots and fetch artifact/design URLs)
Shell:    Bash (scan `.ai/design-critique/` for the next `NNN` when writing the report)

This skill is **read-only toward the reviewed target** — it never edits the design, spec, or code under review. It only writes its own report under `.ai/design-critique/`.
...
### Step 9: Synthesize, Score, and Write the Report

Group findings by severity, assign tiers, lead with the most impactful issues, then write the report per the **Output Convention** below. Every review produces exactly one report file in the same shape — no ad-hoc formats.

---

## Output Convention

Every invocation writes one report, in one shape, whether the target is a spec doc, mockups, screens, or a live UI.

| Artifact | Path | Always Written? |
|---|---|---|
| **Report** | `.ai/design-critique/<NNN>-<topic>/report.md` | **Yes** — every invocation |

**Topic naming:** derive `<topic>` from the reviewed target (its filename, feature, or screen name — e.g. `080-ux-system-and-mockups.md` → topic `ux-system-and-mockups`). Find the next available `NNN` by scanning `.ai/design-critique/` (start at `001`).

**Report shape** — verdict-first, fixed sections:

```markdown
# Design Critique Report — <topic>

**Target:** <path / URL reviewed>
**Date:** <YYYY-MM-DD>
**Scope / Fidelity:** Wireframe | Low-fi mockup | Hi-fi mockup | Prototype | Spec doc | Live UI
**Persona / Primary task:** <who, trying to do what>

## Overall
<verdict in one line — e.g. "Solid IA, but three primary flows are missing error/empty states.">

## Score Card
Per-dimension High / Medium / Low with a one-line reason each:
Heuristics · UX Laws · Interaction/Posture · Information Architecture · Content/Writing · State & Case Coverage · Completeness vs. requirements.

## Findings
Severity-grouped (4 → 0). Each finding:
**Severity:** 0–4  ·  **Principle:** <heuristic / UX law>  ·  **Location:** <where>
**What's wrong** / **Why it matters** / **Recommendation** (specific, actionable).

## What's Working Well
2–4 patterns worth preserving.

## Missing States Check
Table: Default / Hover / Focus / Disabled / Loading / Error / Success / Empty — Present? + notes.
For a spec, also list use-cases/scenarios the spec should cover but doesn't.

## Top 3 Priorities
The three fixes that recover the most user value, in order.

## Residual Risk / Next Steps
What's still unverified, what needs user testing, what to decide next.
```

The full fillable version lives in `templates/design-critique-template.md`; the scored heuristic pass in `templates/heuristic-evaluation-template.md`.

Severity scale is 0–4 (see **Severity Scale for Heuristic Violations** below): 4 Catastrophe · 3 Major · 2 Moderate · 1 Minor · 0 Cosmetic.

## Present Results

After writing the report, present a compact summary to the user — do **not** paste the whole report:

- Lead with the **Overall verdict** (one line).
- The **Score Card** as a short table.
- **Top 3 Priorities**.
- A link to the full report: `.ai/design-critique/<NNN>-<topic>/report.md`.

Close by offering the real next branches (fix the top finding, deep-dive a dimension, re-review after changes), CLI-style. This skill mutates nothing under review, so there is no write-approval gate — the report is the deliverable.
