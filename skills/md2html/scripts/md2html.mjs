...
function stripFrontmatter(source) {
  ...
  return match2 ? source.slice(match2[0].length) : source;
}

// src/images.js
import fs from "node:fs";
import path from "node:path";
var WARN_SINGLE_BYTES = 5 * 1024 * 1024;
...
var FALLBACK_EXTS = [".webp", ".jpg", ".jpeg", ".png", ".gif", ".svg"];
...
function createImageInliner(baseDir) {
  let totalBytes = 0;
  return {
...
function buildRenderer() {
  return new lib_default({
    ...
    linkify: true,
    typographer: true
  }).use(footnote_plugin).use(import_markdown_it_task_lists.default);
}
function firstHeading(markdown) {
  const match2 = markdown.match(/^#{1,6}\s+(.+?)\s*#*\s*$/m);
...
function convert(mdSource, { baseDir, title } = {}) {
...
var USAGE = `md2html \u2014 convert a markdown file to a self-contained HTML file and open it in the browser.
...
function main() {
  ...
  const input = path2.resolve(process.cwd(), positionals[0]);
  ...
  let html;
  try {
    const source = fs2.readFileSync(input, "utf8");
    html = convert(source, {
      baseDir: path2.dirname(input),
      title: path2.basename(input, path2.extname(input))
    });
  } catch (err) {
  ...
  fs2.writeFileSync(output, html, "utf8");
  console.log(output);
  if (!noOpen) {
